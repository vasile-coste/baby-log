    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = { showAddManual = true }) {
                Icon(Icons.Filled.Add, contentDescription = stringResource(R.string.sleep_add_manual_cd))
            }
        },
    ) { padding ->
        Column(
            ...
            SleepSummaryCard(totalMinutes = uiState.totalSleepMinutesToday)

            if (uiState.sleepEntries.isEmpty()) {
                Box(modifier = Modifier.weight(1f).fillMaxWidth(), contentAlignment = Alignment.Center) {
                    Text(stringResource(R.string.sleep_no_entries), style = MaterialTheme.typography.bodyLarge)
                }
            } else {
                val sortedEntries = if (sortAscending) {
                    uiState.sleepEntries.sortedBy { it.startTime }
                } else {
                    uiState.sleepEntries.sortedByDescending { it.startTime }
                }
                LazyColumn(modifier = Modifier.weight(1f).fillMaxWidth()) {
                    ...
                    items(sortedEntries) { entry ->
                        SleepTimelineItem(entry = entry, onLongPress = { editingEntry = entry })
                    }
                }
            }
        }
